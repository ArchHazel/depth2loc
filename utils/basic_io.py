...
def list_images_with_human_presence(cfg: DictConfig):
    ...
    rgb_images = np.array(list_and_sort_files(cfg.model.rgb_F, key=lambda x: int(x.split('.')[0].split('_')[-1])))
    return rgb_images[mask_boolean]



def save_predicted_depth_in_png_and_npy(depth, img_path, cfg: DictConfig):
    os.makedirs(cfg.model.depth_F_npy, exist_ok=True)
    np.save(os.path.join(cfg.model.depth_F_npy, img_path.split('.')[0] + '.npy'), depth)
    os.makedirs(cfg.model.depth_F_png, exist_ok=True)
    visualize_depth(depth, cfg.model.depth_F_png, int(img_path.split('.')[0].split('_')[-1]), cfg.model.to_visual.scaling_factor)
