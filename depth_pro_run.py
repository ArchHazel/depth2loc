...
depth_folder = f'/home/hazel/Datasets/image-online/{movie_name}_depth_vis'
depth_folder_raw = f'/home/hazel/Datasets/image-online/{movie_name}_depth_raw'
...
intrinsic_matrix = np.array([[905,0,640],
                            [0,905,360],
                            [0,0,1]])
...
if not os.path.exists(depth_folder_raw):
    os.makedirs(depth_folder_raw)
if not os.path.exists(visualize_video_folder):
    os.makedirs(visualize_video_folder)
...
for img_path in tqdm(imgs_path, desc="Estimating depth"):
    run = False
    if run:
        image, _, f_px = depth_pro.load_rgb(os.path.join(img_folder, img_path))
...
for img_idx, img_path in tqdm(enumerate(imgs_path), desc="Visualizing depth on image"):
    if img_path in keypoints_3D:
        image = cv2.imread(os.path.join(img_folder, img_path))
